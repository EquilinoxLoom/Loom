...
public class MixinInformationComponentLoader {
    ...
    public ComponentBlueprint load(CSVReader reader, Blueprint actualBlueprint) {
        ...
        try {
            ...
            loom$setField(blueprint, "name", embroider ? name : GameText.getText(Integer.parseInt(name)));

            String description = reader.getNextLabelString();
            loom$setField(blueprint, "description", embroider ? description : GameText.getText(Integer.parseInt(description)));
